...
var isOriginAllowed = /* @__PURE__ */ __name((origin, allowed) => {
  if (!origin) return false;
  if (!allowed) return true;
  return origin === allowed;
}, "isOriginAllowed");
var buildCorsHeaders = /* @__PURE__ */ __name((request, env) => {
...
var stateCookieName = "oauth_state";
var nonceCookieName = "oauth_nonce";
var randomString = /* @__PURE__ */ __name(() => crypto.randomUUID().replace(/-/g, ""), "randomString");
var domainSegment = /* @__PURE__ */ __name((request, env) => {
...
var setSessionCookie = /* @__PURE__ */ __name((sessionId, request, env) => {
  const secure = request.url.startsWith("https:");
  const sameSite = "Lax";
  const domain = domainSegment(request, env);
  return `${sessionCookieName2}=${encodeURIComponent(sessionId)}; HttpOnly; Path=/; SameSite=${sameSite}${domain}${secure ? "; Secure" : ""}`;
}, "setSessionCookie");
var clearSessionCookie = /* @__PURE__ */ __name((request, env) => {
  const secure = request.url.startsWith("https:");
  const sameSite = "Lax";
  const domain = domainSegment(request, env);
  return `${sessionCookieName2}=deleted; Path=/; Max-Age=0; SameSite=${sameSite}${domain}${secure ? "; Secure" : ""}`;
}, "clearSessionCookie");
...
var googleAuthStart = /* @__PURE__ */ __name(async (request, env) => {
  ...
  const redirectUri = env.GOOGLE_REDIRECT_URI?.trim() || new URL("/auth/google/callback", request.url).toString();
...
var decodeJwt = /* @__PURE__ */ __name((token) => {
...
var googleAuthCallback = /* @__PURE__ */ __name(async (request, env) => {
  if (request.method !== "GET") {
    return json({ error: "Method not allowed" }, { status: 405 });
  }
...
// .wrangler/tmp/bundle-soIAS5/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-soIAS5/middleware-loader.entry.ts
